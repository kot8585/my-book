...
export default function ReadingNoteButtons({
  userIdx,
  isbn,
  type,
  title,
  ...
  const handleOnClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    router.push(
      `/timer/${encodeURIComponent(title)}?readingType=${type}&isbn=${isbn}`
    );
  };
  ...
  return (
    <div className="bg-brand-color text-white rounded-full flex items-center justify-center absolute -bottom-5 right-2">
      <SimpleButton className="pl-2 py-2">
        <FaPlay onClick={handleOnClick} className="w-5 h-5 text-gray-50" />
      </SimpleButton>
      <span> | </span>
      <SimpleButton
        className="pr-2 py-2"
